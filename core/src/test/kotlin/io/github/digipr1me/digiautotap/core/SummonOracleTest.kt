...
/**
 * The summon readers against oracle/summon.json: every frame it names,
...
class SummonOracleTest {
    ...
    fun load() {
        ...
        val file = OracleFamilies.pathOf(OracleFamilies.SUMMON, repo)
    ...
    fun `every reader gives the oracle's answer on every frame`() {
        val report = OracleFamilies.check(OracleFamilies.SUMMON, oracle, repo, Vision(ClassPathAssets))
        report.print()
